...
let middleware = function (private_key, passphrase, injection) {
  ...
  function * define(ctx) {
    try {
      ...
      if (injection.header_key) header_key = ctx.request.get(injection.header_key) || false;
      if ( (injection.header_key === header_key) &&  header_key ) {
        let body_check;
