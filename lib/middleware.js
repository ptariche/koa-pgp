...
let middleware_injection = function (_injectedPromiseFunctionReturningKey) {
  ...
  function * define(ctx) {
    try {
      ...
      if (!attempt) {
        ...
        _ctx.response        = ctx.response ? ctx.response : {};
        _ctx.response.status = 406;
        _ctx.response.body   = {};
      }
  ...
  return function *(next) {
    try {
      ...
      debug('The lookup middleware failed to initiate %s', err);
      throw err;
    }
  };
};
  ...
  function * define(ctx) {
    try {
      ...
      if (!lookupKey) {
        ...
        ctx.response.body   = { error: 'Unable to lookup or locate the public key with the PGP Identifier'};
      } else {
...
let middleware = function (private_key, passphrase, injection) {
  ...
  function * define(ctx) {
    try {
          ...
          if (read) {
            ...
            if (injection.pgp_message_key) {
              ...
              try {
                ...
                ctx.response.body   = {error: 'Please provider proper JSON'};
      ...
      this.response.body   = {error: 'An internal server error occured'};
      return;
    }
  };
