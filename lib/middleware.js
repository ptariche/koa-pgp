    ...
    try {
      ...
      if (!injection.header_key || injection.header_key === header_key) {
        ...
        if (ctx && ctx.request && ctx.request.body && body_check ) {
          ...
          let read    = (ctx.request && ctx.request.body) ? yield require('./reader.js')(ctx, private_key, passphrase, message) : false;
          if (read) {
...
let middleware_out = function (public_key, injection) {
  ...
  function * define(ctx) {
    try {
      ...
      let message = injection.pgp_message_key ? ctx.request.body[injection.pgp_message_key] : ctx.request.body;
      debug('Message attempting to be encrypted out %s', message);
      let write   = (ctx.request && ctx.request.body) ? yield require('./writer.js')(ctx, public_key, message) : false;
      if (write) {
